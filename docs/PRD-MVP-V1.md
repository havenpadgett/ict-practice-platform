...
| 2026-09-25 | **OPEN — attempts don't record `session_id`** (PRD Section 9). Found by `tests/data-integrity.test.ts`. | The Supabase move (2026-09-10) kept `SessionState` local and never added the column. Analytics infers sessions from 30-minute gaps instead (`src/lib/analytics.ts`). | Migration `supabase/migrations/20260925120000_attempts_session_id_and_indexes.sql` adds a nullable `session_id` column. **Not yet run.** Once it's applied, `src/lib/attempt-rows.ts` must write `SessionState.session_id`. Writing it before the column exists would make every insert fail, so the code change waits for the migration. | `npm test`: the `session_id` test fails until both land |
