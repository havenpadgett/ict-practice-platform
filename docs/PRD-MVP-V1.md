...
| 2026-09-27 | **Answer keys stay on the server; grading runs in Server Functions (AI-DRAFTED).** The browser gets exercises without keys and gets the key back only with a verdict. Sessions are built from the answer-free catalog. Full reasoning is in docs/ANSWER-KEYS.md. | Every key, including the 50 unreviewed real scenarios, was in the practice page's JavaScript and visible in React DevTools as a prop. For a learning product, peeking turns practice into recall of the key. | Fetch the key after answering and grade in the browser: same latency, weaker, because the key endpoint can be called first. Accept it as a limitation: rejected. **Traded away:** instant verdicts (now one round-trip) and grading without a server connection. **Still exposed:** Free Trade's future candles (Backlog). |
| 2026-09-27 | **Integrity constraints on attempts (AI-DRAFTED).** Migration `20260927130000_attempt_integrity.sql` checks the concept, answer shape, correctness consistency per mode and sane ranges (docs/DATA-INTEGRITY.md). Constraints are added NOT VALID, then validated where existing data allows, so old rows can't fail the migration. The grading thresholds are not encoded. | Users can still write their own rows, and a bug shouldn't be able to store a contradiction. The thresholds are open (D-3), and old rows must stay valid if they change. | Encode the thresholds: rejected, it would invalidate history on re-tuning. Plain (validated) constraints: rejected, one bad historical row would block the whole migration. |
---
...
| 2026-09-27 | Pre-login attempts migrated at sign-in took `answer_type` from today's catalog, not from what the user did. A Liquidity box drawn on 2026-09-09, when Liquidity was still a zone, was stored as a `level` exercise answered with a `region`. Response times were also uncapped. Found while writing the integrity constraints. | `migrateLocalAttempts` (`src/lib/attempts.ts`) looked up the exercise's current answer type. | The answer type now comes from the stored answer (box → zone, line → level). Response time is capped at 24 hours and attempt number is at least 1. Existing rows of this shape are listed by `supabase/audit/attempt_integrity_audit.sql` (docs/DATA-INTEGRITY.md). | `tests/sql/integrity.test.ts`: a migrated 2026-09-09 Liquidity box is recorded as zone/region and accepted by the new constraints. |
