...
- [ ] **(2026-09-13)** Dashboard's "Accuracy by Concept" grid (`src/app/dashboard/page.tsx`) and the concept picker (`src/components/practice/concept-picker.tsx`) both use a fixed 2-column grid — with MSS added as a third concept, the third tile now wraps onto its own row instead of filling out a 3-column layout. Cosmetic only; consider `grid-cols-3` (or responsive column count based on `CONCEPT_LIST.length`) in a cleanup pass.
