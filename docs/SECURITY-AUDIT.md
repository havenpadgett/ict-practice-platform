...
| S13 | **Low** | **The proxy failed open when Supabase config was missing.** Found in the deployment check: production has no Supabase env vars, so `/dashboard` and `/practice` were served without a login check. No data was reachable. | `src/proxy.ts` | **Fixed.** Protected paths now redirect to `/login` when config is missing. |
